	...
	while (1) {
		...
		if (msgReceived) {
			...
			case MAVLINK_MSG_ID_RAW_IMU: {
				...
				printf(
						"Got message IMU RAW (spec: https://pixhawk.ethz.ch/mavlink/#RAW_IMU)\n");
				//printf("\t time: %llu\n", imu.time_usec);
				printf("\t acc  (NED):\t% d\t% d\t% d (m/s^2)\n", imu.xacc,
						imu.yacc, imu.zacc);
				printf("\t gyro (NED):\t% d\t% d\t% d (rad/s)\n", imu.xgyro,
						imu.ygyro, imu.zgyro);
				printf("\t mag  (NED):\t% d\t% d\t% d (Ga)\n", imu.xmag,
						imu.ymag, imu.zmag);
				//printf("\t baro: \t %f (mBar)\n", imu.abs_pressure);
				//printf("\t altitude: \t %f (m)\n", imu.pressure_alt);
			...
			case MAVLINK_MSG_ID_STATUSTEXT: {
				...
				if (strcmp("Initialising APM...", status.text) == 0) {
					mavlink_message_t msg;

					mavlink_msg_request_data_stream_pack(1, 1, &msg, 1, 1,
							MAV_DATA_STREAM_RAW_SENSORS, 1, 1);
					uint8_t buffer[MAVLINK_MSG_ID_REQUEST_DATA_STREAM_LEN];
					unsigned int messageLength = mavlink_msg_to_send_buffer(
							buffer, &msg);
...
int main(int argc, char **argv) {
	...
	mavlink_msg_request_data_stream_pack(1, 1, &msg, 1, 1,
			MAV_DATA_STREAM_RAW_SENSORS, 1, 0);
	uint8_t buffer[MAVLINK_MSG_ID_REQUEST_DATA_STREAM_LEN];
	unsigned int messageLength = mavlink_msg_to_send_buffer(buffer, &msg);
	write(fd, buffer, messageLength);
